...
export default function AuthPage() {
    ...
    return <main style={{ minHeight: "100vh", display: "grid", placeItems: "center", padding: 20 }}><section style={{ width: "min(100%, 420px)", background: "var(--surface)", border: "1px solid var(--line)", borderRadius: 14, padding: 28 }}><div className="brand" style={{ padding: 0, marginBottom: 32 }}><div className="brand-mark"><BookOpenCheck size={17} /></div><span className="brand-name">Lexicon Loop</span></div><div className="eyebrow">Sign in</div><h1 style={{ fontFamily: "Fraunces, serif", fontSize: 34, margin: "8px 0" }}>Keep the words close.</h1><p style={{ color: "var(--muted)", fontSize: 13, lineHeight: 1.55, marginBottom: 24 }}>Use the email and password already registered in Supabase.</p><form className="form-grid" onSubmit={signIn}><div className="field"><label htmlFor="email">Email address</label><div className="input-with-action"><input id="email" type="email" required autoComplete="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="you@example.com" /><span style={{ display: "grid", placeItems: "center", padding: "0 11px", color: "var(--muted)" }}><Mail size={15} /></span></div></div><div className="field"><label htmlFor="password">Password</label><div className="input-with-action"><input id="password" type="password" required autoComplete="current-password" value={password} onChange={(event) => setPassword(event.target.value)} placeholder="Your password" /><span style={{ display: "grid", placeItems: "center", padding: "0 11px", color: "var(--muted)" }}><LockKeyhole size={15} /></span></div></div><button className="primary-button" disabled={loading}>{loading ? "Signing in..." : "Sign in"}<ArrowRight size={15} /></button></form>{message && <p style={{ color: "var(--red)", fontSize: 12, lineHeight: 1.5, margin: "18px 0 0" }}>{message}</p>}</section></main>;
}
